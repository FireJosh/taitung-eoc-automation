...
>
> 這份文件是 **2025 年 12 月專案提案階段** 的初期構想，記錄當時設想的系統範圍。
> 實際開發後，系統範圍與技術路線都與本文件有重大差異，**請勿以本文件理解現行系統**。
>
> **主要差異：**
>
> | 本文件（提案構想） | 實際系統 |
> |---|---|
> | EMIC 同步（Selenium RPA 自動登打） | **EMIC 僅唯讀取用**，且只用於單一欄位的備援 |
> | 核定後自動 LINE 推播／Email 發送 | **對外發布一律人工簽核**，系統不自動對外發送 |
> | 會議記錄自動化（Whisper ASR） | 未納入本系統範圍 |
> | LINE 報案／語音報案系統介接 | 未納入本系統範圍 |
> | GPT-4 / Claude 雲端 API | **本地端 LLM（Ollama）**，機敏資料不出機關網路 |
>
> 現行架構請見 → [`README.md`](../README.md) 與 [`docs/01_architecture.md`](../docs/01_architecture.md)
>
> 保留本文件的目的是保存設計演進紀錄。
